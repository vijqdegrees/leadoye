...
trait DealRelationshipTrait
{
    ...
    public function getLeadTypeAttribute()
    {
        $personLeadType = new \ReflectionClass(Person::class);
        return $this->contextable_type == $personLeadType->getName() ? 1 : 2;
    }
    ...
    {
        $to = $this->updated_at;
        if (!\Cache::has('open_deal_status_id')) {
            \Cache::remember('open_deal_status_id', 36,function () {
                    return Status::findByNameAndType('status_open', 'deal')->id;
            });
        }
        $openDealStatus= \Cache::get('open_deal_status_id');
        if ($this->status_id == $openDealStatus) {
            $to= now();
        }
