...
class DealService extends ApplicationBaseService
{
    ...
    public function showDeal()
    {
        return $deals = $this->model::query()
            ->with([
                ...
                'contextable:id,name,contact_type_id',
                'contextable.contactType:id,name,class',
    ...
    public function proposalWithDeal($deal)
    {
        return Proposal::query()
            ->where('deal_id',$deal->id)
            ->select(['id','subject','deal_id','created_at','created_by','status_id','owner_id'])
            ->with(
                'status:id,name,class',
                'owner:id,first_name,last_name'
            )
            ->paginate(request('per_page', 15), request('page', 1));
    }
    ...
    public function updateDeal($deal, $params)
    {
        ...
        //need to perform any action after update a deal
        if (array_key_exists('action_type', $params) && (request('action_type') == 'deal_lost')) {
            $rfc = new \ReflectionClass(Deal::class);
            $deal->revisionHistory()->insert(
                [
                    'revisionable_type' => $rfc->getNamespaceName() . '\Deal',
                    ...
                    'user_id' => auth()->user()->id,
                    'key' => 'lost_reason_id',
                    'old_value' => request('lost_comment') ?? '',
                    'new_value' => $deal->lost_reason_id,
                    'created_at' => new \DateTime(),
                    'updated_at' => new \DateTime(),
                ]
            );
    ...
    public function notifyAndMailToOwner(Deal $deal, string $event)
    {
        if ($deal->owner_id) {
            $audiences = [$deal->owner_id];
    ...
    public function getDealsByOwner($owner_id)
    {
        return $deals = $this->model::query()
